from defs import Pieces, Side, Sq120to64


Mirror64 = [
    56, 57, 58, 59, 60, 61, 62, 63,
    48, 49, 50, 51, 52, 53, 54, 55,
    40, 41, 42, 43, 44, 45, 46, 47,
    32, 33, 34, 35, 36, 37, 38, 39,
    24, 25, 26, 27, 28, 29, 30, 31,
    16, 17, 18, 19, 20, 21, 22, 23,
     8,  9, 10, 11, 12, 13, 14, 15,
     0,  1,  2,  3,  4,  5,  6,  7,
]

PawnTable = [
    0, 0, 0, 0, 0, 0, 0, 0,
    10, 10, 0, -10, -10, 0, 10, 10,
    5, 0, 0, 5, 5, 0, 0, 5,
    0, 0, 10, 20, 20, 10, 0, 0,
    5, 5, 5, 10, 10, 5, 5, 5,
    10, 10, 10, 20, 20, 10, 10, 10,
    20, 20, 20, 30, 30, 20, 20, 20,
    0, 0, 0, 0, 0, 0, 0, 0,
]

KnightTable = [
    -50, -40, -30, -30, -30, -30, -40, -50,
    -40, -20, 0, 5, 5, 0, -20, -40,
    -30, 5, 10, 15, 15, 10, 5, -30,
    -30, 0, 15, 20, 20, 15, 0, -30,
    -30, 5, 15, 20, 20, 15, 5, -30,
    -30, 0, 10, 15, 15, 10, 0, -30,
    -40, -20, 0, 0, 0, 0, -20, -40,
    -50, -40, -30, -30, -30, -30, -40, -50,
]

BishopTable = [
    -20, -10, -10, -10, -10, -10, -10, -20,
    -10, 5, 0, 0, 0, 0, 5, -10,
    -10, 10, 10, 10, 10, 10, 10, -10,
    -10, 0, 10, 10, 10, 10, 0, -10,
    -10, 5, 5, 10, 10, 5, 5, -10,
    -10, 0, 5, 10, 10, 5, 0, -10,
    -10, 0, 0, 0, 0, 0, 0, -10,
    -20, -10, -10, -10, -10, -10, -10, -20,
]

RookTable = [
     0, 0, 5, 10, 10, 5, 0, 0,
    -5, 0, 0, 0, 0, 0, 0, -5,
    -5, 0, 0, 0, 0, 0, 0, -5,
    -5, 0, 0, 0, 0, 0, 0, -5,
    -5, 0, 0, 0, 0, 0, 0, -5,
    -5, 0, 0, 0, 0, 0, 0, -5,
    25, 25, 25, 25, 25, 25, 25, 25,
     0, 0, 5, 10, 10, 5, 0, 0,
]


def EvalPosition(pos):
    score = pos.material[Side.WHITE] - pos.material[Side.BLACK]

    pce = Pieces.wP
    for pce_num in range(pos.pce_num[pce]):
        sq = pos.p_list[pce][pce_num]
        score += PawnTable[Sq120to64[sq]]

    pce = Pieces.bP
    for pce_num in range(pos.pce_num[pce]):
        sq = pos.p_list[pce][pce_num]
        score -= PawnTable[Mirror64[Sq120to64[sq]]]

    pce = Pieces.wN
    for pce_num in range(pos.pce_num[pce]):
        sq = pos.p_list[pce][pce_num]
        score += KnightTable[Sq120to64[sq]]

    pce = Pieces.bN
    for pce_num in range(pos.pce_num[pce]):
        sq = pos.p_list[pce][pce_num]
        score -= KnightTable[Mirror64[Sq120to64[sq]]]

    pce = Pieces.wB
    for pce_num in range(pos.pce_num[pce]):
        sq = pos.p_list[pce][pce_num]
        score += BishopTable[Sq120to64[sq]]

    pce = Pieces.bB
    for pce_num in range(pos.pce_num[pce]):
        sq = pos.p_list[pce][pce_num]
        score -= BishopTable[Mirror64[Sq120to64[sq]]]

    pce = Pieces.wR
    for pce_num in range(pos.pce_num[pce]):
        sq = pos.p_list[pce][pce_num]
        score += RookTable[Sq120to64[sq]]

    pce = Pieces.bR
    for pce_num in range(pos.pce_num[pce]):
        sq = pos.p_list[pce][pce_num]
        score -= RookTable[Mirror64[Sq120to64[sq]]]

    # Return score from the side-to-move perspective.
    return score if pos.side == Side.WHITE else -score
